...
def fig15_tempering_effect():
    ...
    ax2.plot(withdrawal_epochs, post_dose_auc, color=CRIT_C, lw=2, ls="-",
             alpha=0.5, marker="o", markersize=6, zorder=5)
    ax2.plot(withdrawal_epochs, post_withdrawal_auc, color=CYCLIC_C, lw=3,
             ls="-", alpha=0.8, marker="s", markersize=8, zorder=6)

    # Annotations
    ax2.text(200, 0.79, "Dose:\n0.79", ha="right", color=CRIT_C, fontsize=8)
    ax2.text(200, 0.93, "Withdrawal:\n0.93", ha="right", color=CYCLIC_C, fontsize=8)

    ax2.text(400, 0.50, "Dose:\n0.70", ha="right", color=CRIT_C, fontsize=8)
    ax2.text(400, 0.97, "Withdrawal:\n0.98", ha="right", color=CYCLIC_C, fontsize=8)

    # Improvement arrows
    ax2.annotate("", xy=(400, 0.97), xytext=(400, 0.70),
                arrowprops=dict(arrowstyle="->,head_length=3,head_width=2",
                                color=ORD_C, lw=2.5))
    ax2.text(405, 0.83, "+0.28", color=ORD_C, fontsize=9, fontweight="bold",
             va="center")

    # Phase labels
    ax2.axvspan(0, 200, alpha=0.1, color=ACCENT_C)
    ax2.text(150, 1.05, "Dosing phase\n(cycle pressure ON)", ha="center",
             color=ACCENT_C, fontsize=8, style="italic")

    ax2.axvspan(200, 600, alpha=0.1, color=PURPLE_C)
    ax2.text(400, 1.05, "Withdrawal phase\n(cycle pressure OFF)", ha="center",
             color=PURPLE_C, fontsize=8, style="italic")

    ax2.set_xlabel("Withdrawal epoch", color=TEXT_C, fontsize=11)
    ax2.set_ylabel("K_rr AUC (diagnostic quality)", color=TEXT_C, fontsize=11)
    ...
    # Legend
    ax2.legend(["Post-dose (declining)", "Post-withdrawal (improving)"],
...
def fig17_spectral_sectors():
    ...
    # SU(2) → SU(3)
    fig.text(0.37, 0.925, "→", ha="center", va="center", fontsize=20, color=SUBTLE_C)
    # SU(3) → E-E
    fig.text(0.63, 0.925, "→", ha="center", va="center", fontsize=20, color=SUBTLE_C)
...
def fig18_bridge_diagram():
    ...
    for i, r in enumerate(results1):
        ax.text(x1 + 0.2, y1 + h1 - 1.2 - i * 0.45, f"✓ {r}",
                ha="left", va="center", color=TEXT_C, fontsize=8, family="monospace")
    ...
    x2, y2 = 5.0, 1.2
    box2 = FancyBboxPatch((x2, y2), w1+0.8, h1, boxstyle="round,pad=0.15",
    ...
    arrow1 = FancyArrowPatch((x1 + w1, y1 + h1/2), (x2, y1 + h1/2),
                              ...
                              color=TEXT_C, lw=3, mutation_scale=4)
    ax.add_patch(arrow1)
    ax.text((x1 + w1 + x2) / 2, y1 + h1/2 + 0.3, "spectral\ninvariants",
            ha="center", va="bottom", color=SUBTLE_C, fontsize=9, style="italic")

    # Paper 2 → Paper 3
    arrow2 = FancyArrowPatch((x2 + w1 + 0.8, y1 + h1/2), (x3, y1 + h1/2),
                              arrowstyle="->,head_length=3,head_width=3",
                              color=TEXT_C, lw=3, mutation_scale=4)
    ax.add_patch(arrow2)
    ax.text((x2 + w1 + 0.8 + x3) / 2, y1 + h1/2 + 0.3, "5 building\nblocks",
            ha="center", va="bottom", color=SUBTLE_C, fontsize=9, style="italic")

    # ── Bottom: narrative ──
    ax.text(7, 0.02, "Sequence:  Emergence (P1)  →  Topology (P2)  →  Observables (P3)",
...
def fig19_composability():
    ...
    ax.text(0.5, 0.52, "⊕", ha="center", va="center", fontsize=36, color=ACCENT_C)

    # ── Arrow → ──
    arrow = FancyArrowPatch((0.55, 0.52), (0.7, 0.52),
                            arrowstyle="->,head_length=5,head_width=4",
                            color=TEXT_C, lw=3, mutation_scale=5)
    ax.add_patch(arrow)
    ...
    arrow2 = FancyArrowPatch((1.125, 0.53), (1.125, 0.40),
                              arrowstyle="->,head_length=4,head_width=3",
                              color=SUBTLE_C, lw=2, mutation_scale=5)
    ax.add_patch(arrow2)

    # ── SU(5) match label ──
    ax.text(1.2, 0.38, "R ≈ SM (0.653 vs 0.657)", ha="center", va="center",
            color=SUBTLE_C, fontsize=8, style="italic",
            bbox=dict(facecolor=BG_PANEL, edgecolor=SUBTLE_C, alpha=0.3, boxstyle="round,pad=0.2"))

    # ── Bottom annotation ──
    ax.text(0.58, 0.08, "P7: Ramanujan × Ramanujan → Ramanujan (unanimous, 6/6 definitions)",
...
def fig20_emergence_minima():
    ...
    # ── Arrow down ──
    ax.annotate("", xy=(0.5, 0.68), xytext=(0.5, 0.79),
                arrowprops=dict(arrowstyle="->,head_length=2,head_width=2",
                                color=TEXT_C, lw=2))
    ax.text(0.62, 0.72, "Killing metric\nminimization", ha="center", color=SUBTLE_C,
            fontsize=8, style="italic")
